...
var loc = {
    ...
    "quizCompleted": "You have completed the {tutorial} quiz challenge.",
...
function loadAzureHeader() {
    ...
    var baseUrl = 'https://azure.microsoft.com';
    var scriptUrl = baseUrl + "/" + azureHeaderLocale + "/asset/menujs/";
    var htmlUrl = baseUrl + "/" + azureHeaderLocale + "/asset/header/";
    var htmlPromise = fetch(htmlUrl, { mode: 'cors' }).then(function (response) { return response.text(); });
    return Promise.all([htmlPromise, contentLoaded])
        .then(function (_a) {
        var html = _a[0];
...
function initHeader() {
    ...
    Promise.all(promises).then(function () { return setHeaderLoaded(); });
}
...
var User =  (function (_super) {
    ...
    User.prototype.setAnonymous = function () {
        ...
        this.name = 'anonymous@anonymous.com';
        this.givenName = 'Anonymous';
        this.isAuthenticated = false;
        this.authenticationMethod = 'AAD';
        this.publish(new UserChangedEvent());
    };
    User.prototype.readFromClaims = function (claims) {
        this.id = claims.Claims.DocsId; 
        this.name = claims.Claims.DocsUserEmailId; 
        this.givenName = claims.Claims.DocsUserName; 
        this.isAuthenticated = true;
        this.authenticationMethod = claims.IsAadAuthentication ? 'AAD' : 'MSA'; 
        this.publish(new UserChangedEvent());
    };
    return User;
}(EventBus));
...
function configureJsll(awa, hasCookieConsent) {
    var jsllConfig = {
        ...
        get authMethod() { return user.authenticationMethod === 'MSA' ? 1 : 2; },
        get isLoggedIn() { return user.isAuthenticated; }
    };
...
function renderResultsHeading(container, platform) {
    ...
    if (versionDisplayName !== null && platformId === 'rest') {
        heading.innerHTML = escape$1(moniker) + " " + heading.innerHTML;
    }
...
function setupToc() {
    ...
    if (filterHolder === null || mobileNavButton === null) {
        return;
    }
    if (useMonikerPicker) {
        handleMonikerChange();
        ...
        mobileNavButton.insertAdjacentElement('afterend', picker2);
    }
    if (useApiSearch) {
        initApiSearch();
...
function createToc() {
    ...
    contentLoaded.then(function () {
        ...
        var focusToc = function () { return (document$1.querySelector('.toc a.selected')
            || document$1.querySelector('.toc li a')).focus(); };
        ...
        var collapseOnLinkClicks = function (_a) {
            var target = _a.target;
            if (target instanceof HTMLElement && target.closest('.toc a')) {
                close();
            }
        };
        var mobileQuery = window$1.matchMedia('screen and (max-width: 768px)');
        var close = function () {
            html.remove('sidebar-fullscreen');
            mobileQuery.removeListener(close);
            removeEventListener('focus', constrainFocus);
            removeEventListener('click', collapseOnLinkClicks);
            openButton.focus();
        };
        var closeButton = document$1.querySelector('.mobile-toc-header');
        closeButton.onclick = close;
        var open = function () {
            html.add('sidebar-fullscreen');
            mobileQuery.addListener(close);
            addEventListener('focus', constrainFocus, true);
            if (html.contains('tutorial')) {
                addEventListener('click', collapseOnLinkClicks);
            }
            focusToc();
        };
        var openButton = document$1.querySelector('.mobile-nav-button');
...
function syntaxHighlight(instructions) {
    ...
    var request = { id: nextId$2++, instructions: instructions };
...
function ensureActionPanel() {
    ...
    exitIcon.classList.add('docon');
    exitIcon.classList.add('docon-navigate-close');
    exitIcon.setAttribute('aria-hidden', 'true');
    collapseButton.insertAdjacentElement('afterbegin', exitIcon);
...
if (!checkIsArchived() && ratingPageTemplates.indexOf(msDocs.data.pageTemplate) !== -1 && location$1.hostname !== 'localhost') {
    cookieConsent.then(function () {
        ...
        var environment = location$1.hostname === 'docs.microsoft.com' || location$1.hostname === 'docs.azure.cn' ? 'prod' : 'int';
...
var base = 'https://docsauthpoc.docsauthpoc.p.azurewebsites.net/api/auth';
...
function tryLoadUser() {
    return fetch(claimsUrl, { mode: 'cors', credentials: 'include' }).then(function (response) {
        if (response.ok) {
            return response.json().then(function (claims) { return user.readFromClaims(claims); });
        }
        else {
            user.setAnonymous();
        }
    }, function () { return user.setAnonymous(); });
}
...
function initSignInLinks(container) {
    container.addEventListener('click', function (event) {
        var element = event.target instanceof Element && event.target.closest("." + signInClassName + ", ." + signOutClassName);
        if (element) {
...
function renderQuestion(container, bus, question) {
    ...
    question.choices.forEach(function (choice) {
        ...
        bus.subscribe(QuizCheckAnswer, function (_a) {
            ...
            var gradedChoice = gradedQuestion.choices.filter(function (item) { return item.id === choice.id; })[0];
            if (gradedChoice !== undefined) {
                if (!gradedChoice.isCorrect) {
                    choiceExplanation.hidden = false;
                    if (gradedChoice.explanation !== null) {
...
var userId$1 = null;
var quizId$1 = getMeta('challenge_uid');
...
function renderQuiz(container, quiz, loginUserId, id) {
    container.innerHTML = '';
    ...
    quizTitle.textContent = loc.quizChallenge;
...
function submitQuizChallenge(checkButton, errorLabel, bus) {
    ...
    checkQuiz(userId$1, unitId$1, quizId$1, selectedAnswerData, bus)
        .then(function (data) {
        if (data !== null) {
            if (data.isUpdated) {
                if (data.achievements) {
                    ...
                    displayGreetingDialog(false);
                    reportLearnEvent('achievement-earned', data.achievements[0].uid);
                    reportLearnEvent('module-completed', data.achievements[0].sourceItem.uid);
                }
                else {
                ...
                var primary = document.querySelector('.primary-holder');
                var firstIncorrectQuestion = document.querySelector('.incorrect');
                scrollTo(firstIncorrectQuestion.getBoundingClientRect().top - formContainer.getBoundingClientRect().top, 100, primary);
            }
        }
...
function renderQuizComplete(quizForm, nextUnit) {
    if (quizForm === void 0) { quizForm = null; }
    if (nextUnit === void 0) { nextUnit = null; }
    if (quizForm === null) {
        quizForm = formContainer;
    }
    if (nextUnit === null) {
    ...
    completeText.textContent = loc.quizCompletedText.replace('{tutorial}', tutorialName).replace('{nextUnitName}', nextUnit);
    divBlock.appendChild(completeText);
    var continueButton = document.createElement('button');
    continueButton.classList.add('button-primary');
    continueButton.classList.add('green');
    continueButton.textContent = loc.quizContinueToNext.replace('{nextUnitName}', nextUnit);
    continueButton.addEventListener('click', function (event) {
        event.preventDefault();
    });
    divBlock.appendChild(continueButton);
    quizForm.appendChild(divBlock);
}
...
function displayGreetingDialog(isUnit) {
...
function getUnitProgress(userId, unitId, bus) {
    ...
    return fetchWithTimeout(createRequest(url, init))
        ...
        if (response.status === 204) {
            ...
            bus.publish(new GetUnitProgressErrorResponse(response.status));
        }
        return null;
    });
}
...
function checkQuiz(userId, unitId, quizId, selectedAnswerData, bus) {
...
var GetUnitProgressErrorResponse =  (function () {
    function GetUnitProgressErrorResponse(responseStatus) {
        this.responseStatus = responseStatus;
    }
    return GetUnitProgressErrorResponse;
}());
...
var firstHashId = '';
var lastHashId = '';
var isLastStepCustomized = false;
var displayType = '';
var hasQuiz = false;
var quizHashId = '#quiz';
...
var quizData = null;
var quizForm = null;
var pendingQuizRendering = false;
...
function tutorialPage() {
    ...
    var contentLoadedHandler = function () {
        if (!msDocs.data.context.chromeless) {
            ...
            isLastStepCustomized = hashnames.isLastStepCustomized;
        }
...
function changeSectionInternal() {
    ...
    function showSection(aHash, scrollToSectionTop) {
        ...
        if (msDocs.data.context.chromeless && (aHash === '#step-0' || aHash === '')) {
            aHash = '#step-1';
        }
...
var manageActionColumn = function (aHash, displayType, firstHashId, lastHashId) {
    ...
    switch (displayType) {
        case 'one-column':
            ...
            break;
        case 'two-column':
            if (aHash === firstHashId || aHash === lastHashId && !isLastStepCustomized || aHash === quizHashId) {
                displayPageActions(aHash === lastHashId);
            }
            else {
...
var setFirstAndLastHashNames = function (element) {
    ...
    return {
        ...
        lastHashId: hasQuiz ? tocAnchors[tocAnchors.length - 2].getAttribute('href') : tocAnchors[tocAnchors.length - 1].getAttribute('href'),
        isLastStepCustomized: hasQuiz ? tocAnchors[tocAnchors.length - 2].textContent !== loc.tutorialNextSteps :
            tocAnchors[tocAnchors.length - 1].textContent !== loc.tutorialNextSteps
    };
};
...
function getProgressRemote(unitId) {
    var bus = new EventBus();
    bus.subscribe(GetUnitProgressErrorResponse, function (_a) {
        isUserLoggedin = false;
        updateProgressDisplay(tutorialId);
        if (hasQuiz) {
            if (!isQuizInit) {
                pendingQuizRendering = true;
            }
            else {
                renderQuiz(quizForm, quizData, userId, quizId);
            }
        }
        return;
    });
    getUnitProgress(userId, unitId, bus)
        .then(function (data) {
        if (data) {
            var thisProgress = {};
        ...
        if (hasQuiz) {
            if (checkIsQuizComplete(data)) {
                renderQuizComplete(quizForm, null); 
            }
            else {
                if (!isQuizInit) {
                    pendingQuizRendering = true;
...
function setupQuizForm() {
    ...
    if (hasQuiz) {
        var mainContainer = document$1.querySelector('main');
        var lastSection = mainContainer.lastElementChild;
        var quizSection = document$1.createElement('section');
        quizSection.setAttribute('id', 'quiz');
        quizForm = document$1.createElement('form');
        quizForm.classList.add('quiz-form');
        if (!isQuizInit) {
            quizPromise = initQuiz(quizForm, userId);
        }
        quizSection.appendChild(quizForm);
        mainContainer.insertBefore(quizSection, lastSection);
    }
}
var isQuizInit = false;
function initQuiz(quizForm, loginUserId) {
    return getQuiz(quizId).then(function (data) {
        quizData = data;
        if (!isUserLoggedin || pendingQuizRendering) {
            renderQuiz(quizForm, data, loginUserId, quizId);
            pendingQuizRendering = false;
        }
        isQuizInit = true;
    });
}
function checkIsQuizComplete(unitProgress) {
    if (unitProgress === null) {
        return false;
    }
    var quizItems = unitProgress.items.filter(function (item) { return item.type === 'quiz'; });
    if (quizItems.length === 0) {
        return false;
    }
    return quizItems[0].isCompleted;
}
...
function initDisplayClasses(user$$1, container) {
    var handler = function () { return container.setAttribute('data-authenticated', user$$1.isAuthenticated.toString()); };
    handler();
    user$$1.subscribe(UserChangedEvent, handler);
}

function renderSidebarSignInLink() {
    contentLoaded.then(function () {
        var pageActions = document.querySelector('#page-actions-content .action-list');
        if (!pageActions) {
            return;
        }
        pageActions.insertAdjacentHTML('afterbegin', "\n\t\t\t<li class=\"not-authenticated\">\n\t\t\t\t<a class=\"" + signInClassName + "\" href=\"#\">\n\t\t\t\t\t<!-- this an i element to make the page actions css work -->\n\t\t\t\t\t<i class=\"docon docon-user\" aria-hidden=\"true\"></i>Sign in\n\t\t\t\t</a>\n\t\t\t</li>");
    });
}

function initUserIndicator(user$$1) {
    var li = document.createElement('li');
    li.classList.add('authenticated');
    li.innerHTML = "\n\t\t<a class=\"" + signOutClassName + "\" href=\"#\">\n\t\t\t<!-- this an i element to make the page actions css work -->\n\t\t\t<i class=\"docon docon-user\" aria-hidden=\"true\"></i><span></span>\n\t\t</a>";
    var a = li.firstElementChild;
    var renderUser = function () { return a.lastElementChild.textContent = "Sign out " + user$$1.givenName; };
    renderUser();
    contentLoaded.then(function () {
        var pageActions = document.querySelector('#page-actions-content .action-list');
        if (pageActions) {
            pageActions.insertAdjacentElement('afterbegin', li);
        }
    });
    user$$1.subscribe(UserChangedEvent, renderUser);
}

function initAuth() {
    if (location.hostname === 'docs.microsoft.com' || getMeta('signintest') !== 'true') {
        return;
    }
    initDisplayClasses(user, document.documentElement);
    initSignInLinks(document.documentElement);
    renderSidebarSignInLink();
    initUserIndicator(user);
    tryLoadUser();
}
